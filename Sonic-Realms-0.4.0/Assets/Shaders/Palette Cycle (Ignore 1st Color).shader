Shader "Custom/PaletteCycle (Ignore 1st Color)"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target 
            {
                ...
                float rowHeight = 2.0f / (float)colorMapHeight; // height of each color map row in fraction of texture height
                ...
                for (uint index = 0; index < maxPaletteLength; index++) 
                {
                    ...
                    // Current color in color map index
                    fixed4 colMapIndex = tex2D (colorMap, float2 ((float)index / colorMapWidth, 1));

                    // Did we find a matching color?
                    if (all(abs(int4(col * 255).rgb - int4(colMapIndex * 255).rgb) <= _Threshold)) 
                    {
                        // If so, grab the replacement color for the cycle
                        colorResult = tex2D (colorMap, float2 ((float)index / colorMapWidth, 1 - rowHeight - (rowHeight * cyclePosition)));

                        colorResult = fixed4 (colorResult.r, colorResult.g, colorResult.b, col.a);

                        if (!doLerp) return colorResult;

                        // If we aren't interpolating the cycle, we stop here
                        colorResultNext = tex2D (colorMap, float2 ((float)index / colorMapWidth, 1 - rowHeight - (rowHeight * cyclePositionNext)));

                        colorResultNext = fixed4 (colorResultNext.r, colorResultNext.g, colorResultNext.b, col.a);

                        float lerpAmt = cyclePositionRaw - cyclePosition;

                        return lerp (colorResult, colorResultNext, lerpAmt);
                    }
                }
                return col;
            }
